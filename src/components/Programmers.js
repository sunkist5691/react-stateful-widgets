...
export default function Programmers() {
  ...
  const [curPro, setPro] = useState(listOfAwesome)
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: curPro.id !== undefined ? 'gold' :'royalblue' // 🤔 color turns to gold, when celebrating
  };
  ...
  return (
    <div className='widget-programmers container'>
      ...
      <div className='programmers'>
        {
          /* We should map over a slice of state, instead of 'listOfAwesome'.*/
          listOfAwesome.map( (dev) => 
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => setPro(dev)}>Feature</button> 
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          curPro.id !== undefined ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : 'Pick an awesome programmer'
        }
      </div>
    </div>
  );
